...
## Context
The server is a single bare-metal box with a RTX 2070 Super and modest CPU/RAM. The choice was between (a) installing Debian directly and running k3s + everything else as containers, (b) installing Proxmox VE and running k3s in a VM, or (c) installing Talos Linux as a Kubernetes-native OS.
...
- ⚠️ NVIDIA drivers are mainline on the host; LXC bind-mounts share the host driver, so a host upgrade gone wrong takes down GPU workloads.
...
- **Talos Linux**: beautiful immutable OS, but Turing compatibility requires custom Image Factory schematics that may not be maintained indefinitely. Wrong moment for this hardware.
